...
import Foundation
struct SocialLinkRoot{
    
    var data : [SocialLinkData]!
    var extras : [SocialLinkExtra]!
    var message : String!
    var success : Bool!
    ...
    init(fromDictionary dictionary: [String:Any]){
        ...
        if let dataArray = dictionary["data"] as? [[String:Any]]{
            for dic in dataArray{
                let value = SocialLinkData(fromDictionary: dic)
                data.append(value)
            }
        }
        
        extras = [SocialLinkExtra]()
        if let extrasArray = dictionary["extras"] as? [[String:Any]]{
            for dic in extrasArray{
                let value = SocialLinkExtra(fromDictionary: dic)
                extras.append(value)
            }
        }
