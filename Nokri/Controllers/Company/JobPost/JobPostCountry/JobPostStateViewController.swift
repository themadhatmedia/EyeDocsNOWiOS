...
import UIKit
import Alamofire
protocol LocSecondLevelDelegate: class {
    func sendLocSecondLevelId(locId: Int)
}

class JobPostStateViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,LocThirdLevelDelegate {
    ...
    var key:Int = 0
    var delegate: LocSecondLevelDelegate?
    var ThirdLvlLocId = 0
    ...
    func sendLocThirdLevelId(locId: Int) {
        print(locId)
        self.ThirdLvlLocId = locId
        delegate?.sendLocSecondLevelId(locId: self.ThirdLvlLocId)
    ...
    @objc func btnNext(sender: UIButton!){
        ...
        if sender.currentTitle == "false"{
            ...
            delegate?.sendLocSecondLevelId(locId: sender.tag)
            self.perform(#selector(self.nokri_showNavController1), with: nil, afterDelay: 0)
